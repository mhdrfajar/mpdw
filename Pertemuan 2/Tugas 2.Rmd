...
vif(model)
```
Dengan menggunakan metode vip yang menduga adanya multikol ketika vip >10, diperoleh bahwa terdapat multikol pada variabel x4. sehingga 
```{r}
#Menghapus variabel x4 dari model awal
...
vif(model2)
```
Setelah membuang variabel x4 dari model awal, didapatkan model terbaru dengan vif < 10 pada setiap peubah bebasnya. Artinya tidak terdapat multikol pada model yang terbaru.
...
t.test(model2$residuals,
       ...
       conf.level = 0.95)
```
Hasil uji t menunjukkan bahwa p-value lebih besar dari taraf nyata 5% sehingga disimpulkan bahwa nilai harapan sisaan sama dengan nol.
...
### Ragam sisaan homogen brun pagan dan ncv test
```{r}
# Breusch-Pagan test
library(lmtest)
bptest(model2)
...
bgtest(model2)
```
Berdasarkan plot ACF dan PACF, terlihat terdapat plot yang berada di luar rentang batas yang artinya dapat diperoleh kesimpulan bahwa tolak H0 atau cukup bukti mengatakan adanya autokorelasi. Kesimpulan tersebut juga didukung oleh hasil uji formal menggunakan Runs Test, Durbin Watson Test, dan Breusch-Godfrey Test yang menghasilkan *p-value* \< 0.05. Oleh karena itu, diperlukan penangan autokorelasi. Penanganan yang akan digunakan menggunakan dua metode, yaitu Cochrane-Orcutt dan Hildret-Lu.
